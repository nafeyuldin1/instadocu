    ...
    <>
      {showAnnouncement && (
        <div className={s.announcementBar}>
          ...
          <span>
            Introducing Installatie dosier — Manage all your installation documents in one place.{' '}
      ...
      <nav className={`${s.nav} ${scrolled ? s.navScrolled : ''}`} id="navbar" style={{ top: scrolled ? 0 : (showAnnouncement ? 'var(--announcement-height, 40px)' : 0) }}>
        <div className={s.navInner}>
          ...
          <div className={s.navLinks}>
            ...
            <Link href="/about" className={s.nl}>About</Link>
            <Link href="/blogs" className={s.nl}>Blog</Link>
            <Link href="/contact" className={s.nl}>Contact</Link>
            <Link href="/faqs" className={s.nl}>FAQ</Link>
          </div>
      ...
      <div className={`${s.mobMenu} ${menuOpen ? s.mobMenuOpen : ''}`} id="mobMenu">
        ...
        <div className={s.mobScrollContent}>
          ...
          <Link href="/blogs" onClick={toggleMenu}>Blog</Link>
          <Link href="/contact" onClick={toggleMenu}>Contact</Link>
          <Link href="/faqs" onClick={toggleMenu}>FAQ</Link>
